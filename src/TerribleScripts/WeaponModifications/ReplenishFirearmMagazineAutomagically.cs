...
namespace TerribleScripts.WeaponModifications
{
    public class ReplenishFirearmMagazineAutomagically : MonoBehaviour
    {
        ...
        public void Awake()
        {
            RoundClass = FallbackRoundClass; //so nothing breaks if the gun isnt pre-loaded
        }
        ...
        public void Update()
        {
            ...
            {
                //wish this could be done outside of Update(), but i am not aware of a Chamber Update event or smlr
            ...
            else if (isReplenishing && replenishTimer <= 0)
            {
                ...
                replenishTimer = replenishRate; //the big one
            }
            ...
            {
                hand = firearm.m_hand;
                if (hand.Input.AXButtonDown) isReplenishing = !isReplenishing; // todo: classic
            }
